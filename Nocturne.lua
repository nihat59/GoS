...
local NocturneMenu = Menu("Nocturne", "Nocturne")
NocturneMenu:SubMenu("Combo", "Combo")
NocturneMenu.Combo:Boolean("QCombNoc", "Use Q", true)
NocturneMenu.Combo:Boolean("ECombNoc", "Use E", true)
NocturneMenu.Combo:Boolean("HydraCombNoc", "Use Hydra", true)
NocturneMenu.Combo:BooNocturneMenulean("TiamatCombNoc", "Use Tiamat", true)
NocturneMenu.Combo:Boolean("Blade", "Use BOTRK", true)
NocturneMenu.Combo:Boolean("Bilgewater", "Use Cutlass", true	)
NocturneMenu.Combo:Boolean("RCombNoc", "Use R [ default off , because its situational ]", false)
NocturneMenu.Combo:Slider("MinManaNoc", "Min Mana To Combo",50,0,100,1)

--harass
NocturneMenu:SubMenu("Harass", "Harass", true)
NocturneMenu.Harass:Boolean("QHarassNoc", "Use Q", true)
NocturneMenu.Harass:Boolean("EHarassNoc", "Use E", true)
NocturneMenu.Harass:Slider("HarassMinMana", "Min Mana To Harass",50,0,100,1)

--lc
NocturneMenu:SubMenu("LaneClear", "LaneClear", true)
NocturneMenu.LaneClear:Boolean("Qlc", "Use Q", true)
NocturneMenu.LaneClear:Slider("MinManaLC", "Min Mana To LaneClear",50,0,100,1)

--misc
NocturneMenu:SubMenu("Misc", "Misc")
NocturneMenu.Misc:Boolean("Lvlup", "Use Auto Level", true)
NocturneMenu.Misc:Boolean("Ig", "Use Auto Ignite", true)

--cc
NocturneMenu:SubMenu("AutoW", "Auto W against cc")
NocturneMenu.AutoW:Boolean("CC", "Use W if CC incoming", true)

--pred
NocturneMenu:SubMenu("Prediction", "Prediction Settings")
NocturneMenu.Prediction:Slider("Q", "Hit-Chance: Q" , 30, 0, 99,1)

-- skins
NocturneMenu:SubMenu("SkinChanger", "SkinChanger")
local skinMeta = {["Nocturne"] = {"Classic", "Frozen-Terror", "Void", "Ravager", "Haunted Noc", "Eternum", "Cursed-Revenant"}}
NocturneMenu.SkinChanger:DropDown('skin', myHero.charName.. " Skins", 1, skinMeta[myHero.charName], HeroSkinChanger, true)
NocturneMenu.SkinChanger.skin.callback = function(model) HeroSkinChanger(myHero, model - 1) print(skinMeta[myHero.charName][model] .." ".. myHero.charName .. " Loaded!") end

--drawings
NocturneMenu:SubMenu("Draw", "Drawings")
NocturneMenu.Draw:Boolean("DrawQ", "Draw Q Range", true)
NocturneMenu.Draw:Boolean("DrawW", "Draw W Range", true)
NocturneMenu.Draw:Boolean("DrawE", "Draw E Range", true)
NocturneMenu.Draw:Boolean("DrawR", "Draw R Range", true)
...
-- W
if NocturneMenu.AutoW.CC:Value() then
...
OnTick(function ()
	...
	if NocturneMenu.Misc.Lvlup:Value() then
	...
	if Mode() == "Combo" then
		...
		if NocturneMenu.Combo.QCombNoc:Value() and Ready(_Q) and ValidTarget(target, 1300) then
				if NocturneMenu.Combo.MinManaNoc:Value() <= GetPercentMana then 
				local Qpred = GetPrediction(target, NocQ)
				if Qpred.hitChance >= (NocturneMenu.Prediction.Q:Value() * 0.01) then
					CastSkillShot(_Q,Qpred.castPos)	
				end
			end
		end

		if NocturneMenu.Combo.HydraCombNoc:Value() and Hydra > 0 and Ready(Hydra) and ValidTarget(target, 350) then
			CastSpell(Hydra)
		end

		if NocturneMenu.Combo.TiamatCombNoc:Value() and Tiamat > 0 and Ready(Tiamat) and ValidTarget(target, 350) then
			CastSpell(Tiamat)
		end


		if NocturneMenu.Combo.Blade:Value() and Ready(BOTRK) and ValidTarget(target, 550) then
		...
		if NocturneMenu.Combo.Bilgewater:Value() and Ready(Bilgewater) and ValidTarget(target, 550) then
			if GetPercentHP(myHero) > GetPercentHP(target) and GetDistance(movePos) > GetDistance(target) then
		...
		if NocturneMenu.Combo.ECombNoc:Value() and Ready(_E) and ValidTarget(target, 425) then
			if NocturneMenu.Combo.MinManaNoc:Value() <= GetPercentMana then
			  	CastTargetSpell(target, _E)
			end
		end

		if NocturneMenu.Combo.RCombNoc:Value() and Ready(_R) and ValidTarget(target, 2000) then
			if NocturneMenu.Combo.MinManaNoc:Value() <= GetPercentMana then
	...
	if Mode() == "Harass" then
		...
		if NocturneMenu.Harass.QHarassNoc:Value() and Ready(_Q) and ValidTarget(target, 1300) then
			if NocturneMenu.Harass.HarassMinMana:Value() <= GetPercentMana then
				local Qpred = GetPrediction(target, NocQ)
				if Qpred.hitChance >= (NocturneMenu.Prediction.Q:Value() * 0.01) then
					CastSkillShot(_Q,Qpred.castPos)	
			end
		...
		if NocturneMenu.Harass.EHarassNoc:Value() and Ready(_E) and ValidTarget(target, 425) then
			if NocturneMenu.Harass.HarassMinMana:Value() <= GetPercentMana then
				CastTargetSpell(target, _E)
			end
		end
    ...
    if Mode() == "LaneClear" then
		...
		for _, closeminion in pairs(minionManager.objects) do
			if NocturneMenu.LaneClear.Qlc:Value() and ValidTarget(closeminion, 1300) then
				if GetPercentMP(myHero) >= NocturneMenu.LaneClear.MinManaLC:Value() then
	...
	for _, enemy in pairs(GetEnemyHeroes()) do
	     ...
	     if GetCastName(myHero, SUMMONER_1):lower():find("summonerdot") then
			if NocturneMenu.Misc.Ig:Value() and Ready(SUMMONER_1) and ValidTarget(enemy, 600) then
	     ...
	     if GetCastName(myHero, SUMMONER_2):lower():find("summonerdot") then
			if NocturneMenu.Misc.Ig:Value() and Ready(SUMMONER_2) and ValidTarget(enemy, 600) then
				if GetCurrentHP(enemy) < Ig then
...
OnDraw(function(myHero)
	...
	if NocturneMenu.Draw.DrawQ:Value() then DrawCircle(pos, 1300, 1, 25, GoS.Red) end
	if NocturneMenu.Draw.DrawW:Value() then DrawCircle(pos, 125, 1, 25, GoS.Blue) end
	if NocturneMenu.Draw.DrawE:Value() then DrawCircle(pos, 425, 1, 25, GoS.Black) end
	if NocturneMenu.Draw.DrawR:Value() then DrawCircle(pos, 2000, 1, 25, GoS.Green) end
end)

print("NocturneMenu , dominate the rift!")
