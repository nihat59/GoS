...
PrintChat("AMissFortune loaded.")
PrintChat("by Noddy")

local MissFortuneMenu = Menu("MissFortune", "MissFortune")
MissFortuneMenu:Menu("Combo", "Combo")
MissFortuneMenu.Combo:Boolean("useQ", "Use Q", true)
MissFortuneMenu.Combo:Boolean("useQminion", "Use Q through minion", true)
MissFortuneMenu.Combo:Boolean("useAutoQ", "Use auto Q through dead minion", true)
MissFortuneMenu.Combo:Boolean("useW", "Use W", true)
MissFortuneMenu.Combo:Boolean("useE", "Use E", true)
MissFortuneMenu.Combo:Boolean("useR", "Use R", true)
MissFortuneMenu.Combo:Slider("useRx","Use R if can hit X", 3, 1, 5, 1)
MissFortuneMenu.Combo:Key("Combo1", "Combo", string.byte(" "))
------------------------------------------------------------------
MissFortuneMenu:Menu("Harass", "Harass")
MissFortuneMenu.Harass:Boolean("useQ", "Use Q", true)
MissFortuneMenu.Harass:Boolean("useQminion", "Use Q through minion", true)
MissFortuneMenu.Harass:Key("Harass1", "Harass", string.byte("C"))
------------------------------------------------------------------
MissFortuneMenu:Key("Lasthit1", "Lasthit helper", string.byte("X"))
------------------------------------------------------------------
MissFortuneMenu:Menu("Items", "Items")
MissFortuneMenu.Items:Boolean("useCut", "Bilgewater Cutlass", true)
MissFortuneMenu.Items:Boolean("useBork", "Blade of the Ruined King", true)
MissFortuneMenu.Items:Boolean("useGhost", "Youmuu's Ghostblade", true)
MissFortuneMenu.Items:Boolean("useRedPot", "Elixir of Wrath", true)
------------------------------------------------------------------
...
OnProcessSpell(function(unit,spell)
	if unit == myHero and spell.name:lower():find("attack") and spell.target == GetCurrentTarget() and MissFortuneMenu.Combo.Combo1:Value() and MissFortuneMenu.Combo.useW:Value() and CanUseSpell(myHero,_W) == READY then
		CastSpell(_W)
	end
...
if CanUseSpell(myHero,_Q) == READY then
	if unit == myHero and spell.name:lower():find("attack") and spell.target == target and ValidTarget(target, 650) and MissFortuneMenu.Combo.Combo1:Value() and MissFortuneMenu.Combo.useQ:Value() then
		CastTargetSpell(target,_Q)
	end
	if unit == myHero and spell.name:lower():find("attack") and spell.target == target and ValidTarget(target, 650) and MissFortuneMenu.Harass.Harass1:Value() and MissFortuneMenu.Harass.useQ:Value() then
		CastTargetSpell(target,_Q)
	end
end
end)
...
if MissFortuneMenu.Combo.useAutoQ:Value() then
...
-- Waves
if CanUseSpell(myHero,_R) == READY and MissFortuneMenu.Combo.useR:Value() and ValidTarget(target, 1500) then
...
-- Use Items
if MissFortuneMenu.Combo.Combo1:Value() then
	if CutBlade >= 1 and ValidTarget(target,550) and MissFortuneMenu.Items.useCut:Value() then
		if CanUseSpell(myHero,GetItemSlot(myHero,3144)) == READY then
			CastTargetSpell(target, GetItemSlot(myHero,3144))
		end	
	elseif bork >= 1 and ValidTarget(target,550) and (GetMaxHP(myHero) / GetCurrentHP(myHero)) >= 1.25 and MissFortuneMenu.Items.useBork:Value() then 
		if CanUseSpell(myHero,GetItemSlot(myHero,3153)) == READY then
			CastTargetSpell(target,GetItemSlot(myHero,3153))
		end
	end

	if ghost >= 1 and ValidTarget(target,550+myHitBox) and MissFortuneMenu.Items.useGhost:Value() then
		if CanUseSpell(myHero,GetItemSlot(myHero,3142)) == READY then
			CastSpell(GetItemSlot(myHero,3142))
		end
	end
	
	if redpot >= 1 and ValidTarget(target,550+myHitBox) and MissFortuneMenu.Items.useRedPot:Value() then
...
if CanUseSpell(myHero,_Q) == READY and MissFortuneMenu.Combo.useQ and ValidTarget(target, 650 + myHitBox) and GetCurrentHP(target) - GetDamagePrediction(target,(GetDistance(myHero,target)/1200)*1000) < CalcDamage(myHero, target, 15*GetCastLevel(myHero,_Q) + 5 + 0.85*(GetBaseDamage(myHero)+GetBonusDmg(myHero)), 0) then
	CastTargetSpell(target,_Q)
end
if CanUseSpell(myHero,_Q) == READY and ValidTarget(target, 900) and MissFortuneMenu.Combo.useQ:Value() and MissFortuneMenu.Combo.useQminion:Value() then
	for i,minion in pairs(minionManager.objects) do
		if MINION_ENEMY == GetTeam(minion) then
...
if ULT == false and CanUseSpell(myHero,_E) == READY and ValidTarget(target, 1000) and not IsInDistance(target, 550 + myHitBox) and MissFortuneMenu.Combo.useE:Value() then
...
elseif ULT == false and CanUseSpell(myHero,_E) == READY and ValidTarget(target, 600) and MissFortuneMenu.Combo.useE:Value() then
	local EPred = GetPredictionForPlayer(myHeroPos, target, GetMoveSpeed(target),math.huge, 250, 1000, 1000, false, true)
...
if ULT == false and CanUseSpell(myHero,_R) == READY and ValidTarget(target, 1400) and EnemiesAround(myHeroPos, 550+myHitBox) == 0 and not IsInDistance(target, 550+myHitBox) and MissFortuneMenu.Combo.useR:Value() then
...
for i,enemy in pairs(GetEnemyHeroes()) do
if ULT == false and CanUseSpell(myHero,_R) == READY and ValidTarget(target, 1400) and EnemiesAround(myHeroPos, 550+myHitBox) == 0 and not IsInDistance(target, 550+myHitBox) and MissFortuneMenu.Combo.useR:Value() and CountObjectsOnLineSegment(myHeroPos, (myHeroPos+((VectorWay(myHeroPos,GetOrigin(target))/GetDistance(myHero,target))*1250)), 300, GetEnemyHeroes()) >= MissFortuneMenu.Combo.useRx:Value() and GetCurrentHP(enemy) < CalcDamage(myHero,enemy,0.75*(GetBaseDamage(myHero)+GetBonusDmg(myHero)),0.25*GetBonusAP(myHero))*(2*GetCastLevel(myHero,_R)+5) then
	local RPred = GetPredictionForPlayer(myHeroPos, target, GetMoveSpeed(target),2500, 250, 1400, 350, false, true)
	if RPred.HitChance == 1 then
		IOW.movementEnabled = false
		CastSkillShot(_R,RPred.PredPos)
...
if MissFortuneMenu.Harass.Harass1:Value() then
for i,enemy in pairs(GetEnemyHeroes()) do
if CanUseSpell(myHero,_Q) == READY and ValidTarget(enemy, 900) and MissFortuneMenu.Harass.useQ:Value() and MissFortuneMenu.Harass.useQminion:Value() then
	for i,minion in pairs(minionManager.objects) do
		if MINION_ENEMY == GetTeam(minion) then
			local QPred = GetPredictionForPlayer(myHeroPos, enemy, GetMoveSpeed(enemy),5000, 250, 1000, 0, false, false)
			if QPred.HitChance == 1 then
...
if MissFortuneMenu.Lasthit1:Value() then
	for i,minion in pairs(minionManager.objects) do
		if MINION_ENEMY == GetTeam(minion) then
...
if ValidTarget(target,1400) and MissFortuneMenu.Combo.useR:Value() and wave ~= nil and CanUseSpell(myHero,_R) == READY then	
